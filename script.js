...
if (characterString == "") {
  ...
  location.reload() //checks to make sure at least one character set was seleted. If not, reloads website to reprompt.
}
...
function generatePassword() { //this creates the password
  ...
  for (let i = 0;  i < passwordLength;  i++) {
    let charNumber = Math.floor((Math.random() * characterString.length) + 1) //generates a random number
    slicedChar = characterString.slice(charNumber - 1, charNumber) //slices out a character from characterString based on random number
